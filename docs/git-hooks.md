...
```bash
pnpm exec husky set .husky/pre-commit "pnpm exec secret-scanner scan --staged"
```
...
```bash
pnpm exec husky set .husky/pre-commit "pnpm exec secret-scanner scan --staged && pnpm lint && pnpm test"
```
...
```bash
pnpm exec secret-scanner scan --all --include-untracked
```
...
```bash
pnpm exec secret-scanner redact --all --dry-run
pnpm exec secret-scanner redact --all --apply
```
